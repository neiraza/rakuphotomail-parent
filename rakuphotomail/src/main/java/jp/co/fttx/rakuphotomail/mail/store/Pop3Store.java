...
public class Pop3Store extends Store {
    ...
    class Pop3Folder extends Folder {
        ...
        throws MessagingException, IOException {
            ...
            for (String uid : uids) {
                if (mUidToMsgMap.get(uid) == null) {
                    if (RakuPhotoMail.DEBUG && RakuPhotoMail.DEBUG_PROTOCOL_POP3) {
                        Log.d(RakuPhotoMail.LOG_TAG, "Need to index UID " + uid);
                    }
            ...
            while ((response = readLine()) != null) {
                ...
                if (uidParts.length >= 2) {
                    ...
                    if (unindexedUids.contains(msgUid)) {
                        if (RakuPhotoMail.DEBUG && RakuPhotoMail.DEBUG_PROTOCOL_POP3) {
                            Log.d(RakuPhotoMail.LOG_TAG, "Got msgNum " + msgNum + " for UID " + msgUid);
                        }
        ...
        private void indexMessage(int msgNum, Pop3Message message) {
            if (RakuPhotoMail.DEBUG && RakuPhotoMail.DEBUG_PROTOCOL_POP3) {
                Log.d(RakuPhotoMail.LOG_TAG, "Adding index for UID " + message.getUid() + " to msgNum " + msgNum);
            }
        ...
        throws IOException, MessagingException {
            ...
            if (lines != -1 && (!mTopNotSupported || mCapabilities.top)) {
                try {
                    if (RakuPhotoMail.DEBUG && RakuPhotoMail.DEBUG_PROTOCOL_POP3 && !mCapabilities.top) {
                        Log.d(RakuPhotoMail.LOG_TAG, "This server doesn't support the CAPA command. " +
                    ...
                    if (mCapabilities.top) {
                        ...
                        if (RakuPhotoMail.DEBUG && RakuPhotoMail.DEBUG_PROTOCOL_POP3) {
                            Log.d(RakuPhotoMail.LOG_TAG, "The server really doesn't support the TOP " +
        ...
        private String readLine() throws IOException {
            ...
            if (RakuPhotoMail.DEBUG && RakuPhotoMail.DEBUG_PROTOCOL_POP3) {
                Log.d(RakuPhotoMail.LOG_TAG, "<<< " + ret);
            }
            return ret;
        }
                ...
                if (command != null) {
                    if (RakuPhotoMail.DEBUG && RakuPhotoMail.DEBUG_PROTOCOL_POP3) {
                        if (sensitive && !RakuPhotoMail.DEBUG_SENSITIVE) {
                            ...
                            Log.d(RakuPhotoMail.LOG_TAG, ">>> " + command);
                        }
                    }
...
//         catch (MessagingException me)
//         {
//          Log.w(RakuPhotoMail.LOG_TAG, "Could not delete non-existant message", me);
//         }
        }
    }

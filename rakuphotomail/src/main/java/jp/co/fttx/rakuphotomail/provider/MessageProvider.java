...
public class MessageProvider extends ContentProvider {
	...
	public static interface MessageColumns extends BaseColumns {
		...
		/**
		 * @deprecated the field value is misnamed/misleading - present for
		 *             compatibility purpose only. To be removed.
		 */
		@Deprecated
		String INCREMENT = "id";
	}
	...
	public static class DeleteUriExtractor implements FieldExtractor<MessageInfoHolder, String> {
		...
		public String getField(final MessageInfoHolder source) {
			...
			return CONTENT_URI + "/delete_message/" + message.getFolder().getAccount().getAccountNumber()
	...
	protected class MessagesQueryHandler implements QueryHandler {
		...
		protected MatrixCursor getMessages(final String[] projection) throws InterruptedException {
			...
			// new code for integrated inbox, only execute this once as it will
			// be processed afterwards via the listener
			final SearchAccount integratedInboxAccount = new SearchAccount(getContext(), true, null, null);
			final MessagingController msgController = MessagingController.getInstance(RakuPhotoMail.app);
			...
			final LinkedHashMap<String, FieldExtractor<MessageInfoHolder, ?>> extractors = resolveMessageExtractors(
					projectionToUse, holders.size());
		...
		// returns LinkedHashMap (rather than Map) to emphasize the inner
		// element ordering
	...
	protected static class MonitoredCursor implements CrossProcessCursor {
		 ...
		 * @param semaphore
		 *            The semaphore to release on close. Never <code>null</code>
		 *            .
		 */
		...
		public void close() {
			if (mClosed.compareAndSet(false, true)) {
				mCursor.close();
				Log.d(RakuPhotoMail.LOG_TAG, "Cursor closed, null'ing & releasing semaphore");
				mCursor = null;
				mSemaphore.release();
			}
		}
	...
	protected class ThrottlingQueryHandler implements QueryHandler {
		...
		public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
			...
			/*
			 * Android content resolvers can only process CrossProcessCursor
			 * instances
			 */
			if (!(cursor instanceof CrossProcessCursor)) {
				Log.w(RakuPhotoMail.LOG_TAG, "Unsupported cursor, returning null: " + cursor);
				return null;
			}
			...
			mScheduledPool.schedule(new Runnable() {
				...
				public void run() {
					...
					if (monitored != null && !monitored.isClosed()) {
						Log.w(RakuPhotoMail.LOG_TAG, "Forcibly closing remotely exposed cursor");
						try {
							monitored.close();
						} catch (Exception e) {
							Log.w(RakuPhotoMail.LOG_TAG, "Exception while forcibly closing cursor", e);
						}
					}
				}
			}, 30, TimeUnit.SECONDS);
	...
	protected class MesssageInfoHolderRetrieverListener extends MessagingListener {
		...
		@Override
		public void listLocalMessagesAddMessages(final Account account, final String folderName,
				final List<Message> messages) {
			// cache fields into local variables for faster access on JVM
			// without JIT
			...
			for (final Message message : messages) {
				...
				helper.populate(messageInfoHolder, message, new FolderInfoHolder(context, messageFolder,
		...
		public void searchStats(AccountStats stats) {
			try {
				...
				Log.e(RakuPhotoMail.LOG_TAG, "Unable to return message list back to caller", e);
			}
		}
	}

	public static final String AUTHORITY = "jp.co.fttx.rakuphotomail.messageprovider";

	public static final Uri CONTENT_URI = Uri.parse("content://" + AUTHORITY);

	private static final String[] DEFAULT_MESSAGE_PROJECTION = new String[] { MessageColumns._ID,
			MessageColumns.SEND_DATE, MessageColumns.SENDER, MessageColumns.SUBJECT, MessageColumns.PREVIEW,
			MessageColumns.ACCOUNT, MessageColumns.URI, MessageColumns.DELETE_URI };
	...
	public boolean onCreate() {
		...
		RakuPhotoMail.registerApplicationAware(new RakuPhotoMail.ApplicationAware() {
			@Override
			public void initializeComponent(final RakuPhotoMail application) {
				Log.v(RakuPhotoMail.LOG_TAG, "Registering content resolver notifier");
	...
	public int delete(Uri uri, String selection, String[] selectionArgs) {
		if (RakuPhotoMail.app == null) {
			return 0;
		}

		if (RakuPhotoMail.DEBUG) {
			Log.v(RakuPhotoMail.LOG_TAG, "MessageProvider/delete: " + uri);
		}
		...
		Message msg = null;
		try {
			Folder lf = LocalStore.getLocalInstance(myAccount, RakuPhotoMail.app).getFolder(folderName);
			int msgCount = lf.getMessageCount();
			if (RakuPhotoMail.DEBUG) {
				Log.d(RakuPhotoMail.LOG_TAG, "folder msg count = " + msgCount);
			}
			msg = lf.getMessage(msgUid);
		} catch (MessagingException e) {
			Log.e(RakuPhotoMail.LOG_TAG, "Unable to retrieve message", e);
		}

		// launch command to delete the message
		if ((myAccount != null) && (msg != null)) {
			MessagingController.getInstance(RakuPhotoMail.app).deleteMessages(new Message[] { msg }, null);
		}
	...
	public String getType(Uri uri) {
		...
		if (RakuPhotoMail.DEBUG) {
			Log.v(RakuPhotoMail.LOG_TAG, "MessageProvider/getType: " + uri);
		}
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		if (RakuPhotoMail.DEBUG) {
			Log.v(RakuPhotoMail.LOG_TAG, "MessageProvider/insert: " + uri);
		}
	...
	public Cursor query(final Uri uri, final String[] projection, final String selection,
		...
		if (RakuPhotoMail.DEBUG) {
			Log.v(RakuPhotoMail.LOG_TAG, "MessageProvider/query: " + uri);
		}
		...
		try {
			...
			Log.e(RakuPhotoMail.LOG_TAG, "Unable to execute query for URI: " + uri, e);
			return null;
		}
	...
	public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
		...
		if (RakuPhotoMail.DEBUG) {
			Log.v(RakuPhotoMail.LOG_TAG, "MessageProvider/update: " + uri);
		}

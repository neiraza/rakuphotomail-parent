...
import jp.co.fttx.rakuphotomail.preferences.Editor;
import jp.co.fttx.rakuphotomail.preferences.Storage;
...
public class Preferences {
    ...
    private Preferences(Context context) {
        ...
        mContext = context;
        if (mStorage.size() == 0) {
            Log.i(RakuPhotoMail.LOG_TAG, "Preferences storage is zero-size, importing from Android-style preferences");
    ...
    public synchronized Account newAccount() {
        newAccount = new Account(RakuPhotoMail.app);
    ...
    public void dump() {
        if (Config.LOGV) {
            for (String key : getPreferences().getAll().keySet()) {
                Log.v(RakuPhotoMail.LOG_TAG, key + " = " + getPreferences().getAll().get(key));
            }
        }
    }

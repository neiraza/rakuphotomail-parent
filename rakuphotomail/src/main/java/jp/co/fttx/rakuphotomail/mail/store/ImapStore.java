...
public class ImapStore extends Store {
    ...
    private List<? extends Folder> listFolders(ImapConnection connection, boolean LSUB) throws IOException,
            MessagingException {
        String commandResponse = LSUB ? "LSUB" : "LIST";

        LinkedList<Folder> folders = new LinkedList<Folder>();

        List<ImapResponse> responses = connection.executeSimpleCommand(String.format(commandResponse
                + " \"\" %s", encodeString(getCombinedPrefix() + "*")));
        ...
        for (ImapResponse response : responses) {
            if (ImapResponseParser.equalsIgnoreCase(response.get(0), commandResponse)) {
                ...
                if (folder.equalsIgnoreCase(mAccount.getInboxFolderName())) {
                    ...
                    /*
                          * There is a folder on the server with the same name as our
                          * local outbox. Until we have a good plan to deal with this
                          * situation we simply ignore the folder on the server.
                          */
                    continue;
                } else {
    ...
    public void checkSettings() throws MessagingException {
        try {
            ...
            throw new MessagingException(RakuPhotoMail.app.getString(R.string.error_unable_to_connect), ioe);
        }
    }
    ...
    private String decodeFolderName(String name) throws CharacterCodingException {
        /*
           ...
           */
        try {
            // Make sure the decoder throws an exception if it encounters an
            // invalid encoding.
            CharsetDecoder decoder = mModifiedUtf7Charset.newDecoder().onMalformedInput(
    ...
    class ImapFolder extends Folder {
        ...
        protected List<ImapResponse> executeSimpleCommand(String command) throws MessagingException,
                IOException {
        ...
        public List<ImapResponse> internalOpen(OpenMode mode) throws MessagingException {
            ...
            try {
                ...
                Log.e("kierkegaard", "ImapFolder#internalOpen 4.14 ", ioe);
                throw ioExceptionHandler(mConnection, ioe);
            } catch (MessagingException me) {
                Log.e(RakuPhotoMail.LOG_TAG, "Unable to open connection for " + getLogId(), me);
                throw me;
            }
         ...
         *
         * @param folderName The name of the folder encoded as quoted string. See
        ...
        public void copyMessages(Message[] messages, Folder folder) throws MessagingException {
            ...
            try {
                ...
                if (!exists(remoteDestName)) {
                    /*
                          * If the remote trash folder doesn't exist we try to create
                          * it.
                          */
                    if (RakuPhotoMail.DEBUG)
                        Log.i(RakuPhotoMail.LOG_TAG,
                                "IMAPMessage.copyMessages: attempting to create remote '" + remoteDestName
                                        + "' folder for " + getLogId());
                    iFolder.create(FolderType.HOLDS_MESSAGES);
                }

                if (exists(remoteDestName)) {
                    executeSimpleCommand(String.format("UID COPY %s %s", Utility.combine(uids, ','),
                            remoteDestName));
                } else {
                    throw new MessagingException(
                            "IMAPMessage.copyMessages: remote destination folder " + folder.getName()
            ...
            if (trashFolderName == null || getName().equalsIgnoreCase(trashFolderName)) {
                ...
                if (!exists(remoteTrashName)) {
                    /*
                          ...
                          */
                    if (RakuPhotoMail.DEBUG)
                        Log.i(RakuPhotoMail.LOG_TAG, "IMAPMessage.delete: attempting to create remote '"
                ...
                if (exists(remoteTrashName)) {
                    if (RakuPhotoMail.DEBUG)
                        Log.d(RakuPhotoMail.LOG_TAG, "IMAPMessage.delete: copying remote " + messages.length
                                + " messages to '" + trashFolderName + "' for " + getLogId());
        ...
        private int getRemoteMessageCount(String criteria) throws MessagingException {
            ...
            try {
                ...
                List<ImapResponse> responses = executeSimpleCommand(String.format("SEARCH %d:* " + criteria,
                        start));
                for (ImapResponse response : responses) {
        ...
        protected int getHighestUid() {
            try {
                ...
                Log.e(RakuPhotoMail.LOG_TAG, "Unable to find highest UID in folder " + getName(), e);
            }
            return -1;
        ...
        protected Message[] getMessages(final List<Integer> mesgSeqs, final boolean includeDeleted,
        ...
        protected Message[] getMessagesFromUids(final List<String> mesgUids, final boolean includeDeleted,
                                                final MessageRetrievalListener listener) throws MessagingException {
            ImapSearcher searcher = new ImapSearcher() {
                public List<ImapResponse> search() throws IOException, MessagingException {
        ...
        private Message[] search(ImapSearcher searcher, MessageRetrievalListener listener)
                throws MessagingException {
        ...
        public void fetch(Message[] messages, FetchProfile fp, MessageRetrievalListener listener)
            ...
            for (int windowStart = 0; windowStart < messages.length; windowStart += (FETCH_WINDOW_SIZE)) {
                ...
                try {
                    mConnection
                            .sendCommand(
                                    String.format("UID FETCH %s (%s)", Utility.combine(
                        ...
                        if (response.mTag == null
                            ...
                            if (uid != null) {
                                try {
                                    ...
                                    if (RakuPhotoMail.DEBUG) {
                                        Log.v(RakuPhotoMail.LOG_TAG, "Stored uid '" + uid + "' for msgSeq "
                                                + msgSeq + " into map " /*
																		 * +
																		 * msgSeqUidMap
																		 * .
                                    ...
                                    Log.e(RakuPhotoMail.LOG_TAG, "Unable to store uid '" + uid
                                            + "' for msgSeq " + msgSeq);
                                }
                            }
                            ...
                            if (message == null) {
                                if (RakuPhotoMail.DEBUG)
                                    Log.d(RakuPhotoMail.LOG_TAG, "Do not have message in messageMap for UID "
                                            + uid + " for " + getLogId());
        ...
        public void fetchPart(Message message, Part part, MessageRetrievalListener listener)
            ...
            try {
                mConnection.sendCommand(String.format("UID FETCH %s (UID %s)", message.getUid(), fetch),
                        false);
                ...
                do {
                    ...
                    if ((response.mTag == null)
                        ...
                        if (!message.getUid().equals(uid)) {
                            if (RakuPhotoMail.DEBUG)
                                Log.d(RakuPhotoMail.LOG_TAG, "Did not ask for UID " + uid + " for "
                                        + getLogId());
            ...
            if (fetchList.containsKey("BODYSTRUCTURE")) {
                ...
                if (bs != null) {
                    try {
                        ...
                        if (RakuPhotoMail.DEBUG)
                            Log.d(RakuPhotoMail.LOG_TAG, "Error handling message for " + getLogId(), e);
                    ...
                    if (bracketed.size() > 1) {
                        ...
                        if (keyObj instanceof String) {
                            ...
                            if ("UIDNEXT".equalsIgnoreCase(key)) {
                                ...
                                if (RakuPhotoMail.DEBUG)
                                    Log.d(RakuPhotoMail.LOG_TAG, "Got UidNext = " + uidNext + " for "
                                            + getLogId());
                            }
                        }
                    }
        ...
        protected void handleUntaggedResponse(ImapResponse response) {
            if (response.mTag == null && response.size() > 1) {
                if (ImapResponseParser.equalsIgnoreCase(response.get(1), "EXISTS")) {
                    ...
                    if (RakuPhotoMail.DEBUG)
                        Log.d(RakuPhotoMail.LOG_TAG, "Got untagged EXISTS with value " + mMessageCount
                                + " for " + getLogId());
                }
                ...
                if (ImapResponseParser.equalsIgnoreCase(response.get(1), "EXPUNGE") && mMessageCount > 0) {
                    mMessageCount--;
                    if (RakuPhotoMail.DEBUG)
                        Log.d(RakuPhotoMail.LOG_TAG, "Got untagged EXPUNGE with mMessageCount "
                ...
                // }
                //
                // Log.w(RakuPhotoMail.LOG_TAG, "ALERT: " + sb.toString() +
                // " for " + getLogId());
                // }
                // }
                // }
            ...
            // Log.i(RakuPhotoMail.LOG_TAG, "mMessageCount = " + mMessageCount +
            // " for " + getLogId());
        }
        ...
        private void parseBodyStructure(ImapList bs, Part part, String id) throws MessagingException {
            if (bs.get(0) instanceof ImapList) {
                ...
                /*
                     ...
                     * parenthesized list 3| 3 body id (unused) 4| 4 body
                     * description (unused) 5| 5 body encoding 6| 6 body size -| 7
                     * text lines (only for type TEXT, unused) Extensions
                     * (optional): 7| 8 body MD5 (unused) 8| 9 body disposition 9|10
                     * body language (unused) 10|11 body location (unused)
                     */
                ...
                if (MimeUtility.mimeTypeMatches(mimeType, "message/rfc822")) {
                    ...
                    // [MESSAGE, RFC822, [NAME, Fwd: [#HTR-517941]: update plans
                    // at 1am Friday - Memory allocation - displayware.eml],
                    // NIL, NIL, 7BIT, 5974, NIL, [INLINE, [FILENAME*0, Fwd:
                    // [#HTR-517941]: update plans at 1am Friday - Memory all,
                ...
                /*
                     * Set the content type with as much information as we know
                     * right now.
                     */
                    ...
                    if ((bodyDisposition.size() > 1) && (bodyDisposition.get(1) instanceof ImapList)) {
                        ...
                        for (int i = 0, count = bodyDispositionParams.size(); i < count; i += 2) {
                            contentDisposition += String.format(";\n %s=\"%s\"", bodyDispositionParams
                                    .getString(i).toLowerCase(), bodyDispositionParams.getString(i + 1));
                        }
                    }
                }
         ...
         * implementation also determines the new UID of the given message on
...
//                    }
        ...
        public String getUidFromMessageId(Message message) throws MessagingException {
            ...
            try {
                ...
                if (messageIdHeader == null || messageIdHeader.length == 0) {
                    if (RakuPhotoMail.DEBUG)
                        Log.d(RakuPhotoMail.LOG_TAG,
                                "Did not get a message-id in order to search for UID  for " + getLogId());
                    return null;
                }
                ...
                List<ImapResponse> responses = executeSimpleCommand(String.format(
                        "UID SEARCH HEADER MESSAGE-ID %s", messageId));
        ...
        public void setFlags(Flag[] flags, boolean value) throws MessagingException {
            ...
            try {
                executeSimpleCommand(String.format("UID STORE 1:* %sFLAGS.SILENT (%s)", value ? "+" : "-",
                        combineFlags(flags)));
            } catch (IOException ioe) {
                throw ioExceptionHandler(mConnection, ioe);
            }
        ...
        public String getNewPushState(String oldPushStateS, Message message) {
            try {
                ...
                Log.e(RakuPhotoMail.LOG_TAG, "Exception while updated push state for " + getLogId(), e);
                return null;
            }
        }
        ...
        private MessagingException ioExceptionHandler(ImapConnection connection, IOException ioe) {
            Log.e(RakuPhotoMail.LOG_TAG, "IOException for " + getLogId(), ioe);
            if (connection != null) {
                connection.close();
            }
            close();
    ...
    public static class ImapConnection {
            ...
            for (ImapResponse response : responses) {
                ...
                if (capabilityList != null) {
                    if (capabilityList.size() > 0
                        ...
                        if (RakuPhotoMail.DEBUG) {
                            Log.d(RakuPhotoMail.LOG_TAG, "Saving " + capabilityList.size()
        ...
        public void open() throws IOException, MessagingException {
            ...
            try {
                ...
                Log.w(RakuPhotoMail.LOG_TAG, "Could not set DNS ttl to 0 for " + getLogId(), e);
            }

            try {
                Security.setProperty("networkaddress.cache.negative.ttl", "0");
            } catch (Exception e) {
                Log.w(RakuPhotoMail.LOG_TAG, "Could not set DNS negative ttl to 0 for " + getLogId(), e);
            }
            ...
            try {
                ...
                if (mSettings.getConnectionSecurity() == CONNECTION_SECURITY_SSL_REQUIRED
                    ...
                    sslContext.init(null,
                            new TrustManager[]{TrustManagerFactory.get(mSettings.getHost(), secure)},
                            new SecureRandom());
                ...
                if (RakuPhotoMail.DEBUG && RakuPhotoMail.DEBUG_PROTOCOL_IMAP)
                    Log.v(RakuPhotoMail.LOG_TAG, getLogId() + "<<<" + nullResponse);
                ...
                if (!hasCapability(CAPABILITY_CAPABILITY)) {
                    if (RakuPhotoMail.DEBUG)
                        Log.i(RakuPhotoMail.LOG_TAG,
                                "Did not get capabilities in banner, requesting CAPABILITY for " + getLogId());
                ...
                if (mSettings.getConnectionSecurity() == CONNECTION_SECURITY_TLS_OPTIONAL
                    ...
                    if (hasCapability("STARTTLS")) {
                        ...
                        sslContext.init(null,
                                new TrustManager[]{TrustManagerFactory.get(mSettings.getHost(), secure)},
                                new SecureRandom());
                        mSocket = sslContext.getSocketFactory().createSocket(mSocket, mSettings.getHost(),
                ...
                if (RakuPhotoMail.DEBUG) {
                    Log.d(RakuPhotoMail.LOG_TAG, CAPABILITY_COMPRESS_DEFLATE + " = "
                            + hasCapability(CAPABILITY_COMPRESS_DEFLATE));
                }
                if (hasCapability(CAPABILITY_COMPRESS_DEFLATE)) {
                    ConnectivityManager connectivityManager = (ConnectivityManager) RakuPhotoMail.app
                    ...
                    if (netInfo != null) {
                        ...
                        if (RakuPhotoMail.DEBUG)
                            Log.d(RakuPhotoMail.LOG_TAG, "On network type " + type);
                        useCompression = mSettings.useCompression(type);

                    }
                    if (RakuPhotoMail.DEBUG)
                        Log.d(RakuPhotoMail.LOG_TAG, "useCompression " + useCompression);
                    if (useCompression) {
                        try {
                            ...
                            if (RakuPhotoMail.DEBUG) {
                                Log.i(RakuPhotoMail.LOG_TAG, "Compression enabled for " + getLogId());
                            }
                        } catch (Exception e) {
                            Log.e(RakuPhotoMail.LOG_TAG, "Unable to negotiate compression", e);
                        }
                    }
                }

                if (RakuPhotoMail.DEBUG)
                    Log.d(RakuPhotoMail.LOG_TAG, "NAMESPACE = " + hasCapability(CAPABILITY_NAMESPACE)
                    ...
                    if (hasCapability(CAPABILITY_NAMESPACE)) {
                        ...
                        for (ImapResponse response : namespaceResponses) {
                            if (ImapResponseParser.equalsIgnoreCase(response.get(0), COMMAND_NAMESPACE)) {
                                if (RakuPhotoMail.DEBUG)
                                    Log.d(RakuPhotoMail.LOG_TAG, "Got NAMESPACE response " + response
                                            + " on " + getLogId());

                                Object personalNamespaces = response.get(1);
                                if (personalNamespaces != null && personalNamespaces instanceof ImapList) {
                                    if (RakuPhotoMail.DEBUG)
                                        Log.d(RakuPhotoMail.LOG_TAG, "Got personal namespaces: "
                                                + personalNamespaces);
                                    ...
                                    if (firstNamespace != null && firstNamespace instanceof ImapList) {
                                        if (RakuPhotoMail.DEBUG)
                                            Log.d(RakuPhotoMail.LOG_TAG, "Got first personal namespaces: "
                                                    + firstNamespace);
                    ...
                    try {
                        ...
                        for (ImapResponse response : nameResponses) {
                            if (ImapResponseParser.equalsIgnoreCase(response.get(0), "LIST")) {
                                ...
                                if (RakuPhotoMail.DEBUG)
                                    Log.d(RakuPhotoMail.LOG_TAG,
                                            "Got path delimeter '" + mSettings.getPathDelimeter() + "' for "
                                                    + getLogId());
                            }
                        }
                    } catch (Exception e) {
                        Log.e(RakuPhotoMail.LOG_TAG, "Unable to get path delimeter using LIST", e);
                    }
                }
                ...
                if (tokens != null && tokens.length > 1 && tokens[1] != null) {
                    Log.e(RakuPhotoMail.LOG_TAG, "Stripping host/port from ConnectionException for "
                            + getLogId(), ce);
                ...
                if (!authSuccess) {
                    Log.e(RakuPhotoMail.LOG_TAG, "Failed to login, closing connection for " + getLogId());
                    close();
                }
            }
        }
        ...
        protected boolean isIdleCapable() {
            if (RakuPhotoMail.DEBUG)
                Log.v(RakuPhotoMail.LOG_TAG, "Connection " + getLogId() + " has " + capabilities.size()
                        + " capabilities");
        ...
        public ImapResponse readResponse(ImapResponseParser.IImapResponseCallback callback)
            ...
            try {
                ...
                if (RakuPhotoMail.DEBUG && RakuPhotoMail.DEBUG_PROTOCOL_IMAP)
                    Log.v(RakuPhotoMail.LOG_TAG, getLogId() + "<<<" + response);
        ...
        public void sendContinuation(String continuation) throws IOException {
            ...
            if (RakuPhotoMail.DEBUG && RakuPhotoMail.DEBUG_PROTOCOL_IMAP)
                Log.v(RakuPhotoMail.LOG_TAG, getLogId() + ">>> " + continuation);
            ...
            try {
                ...
                if (RakuPhotoMail.DEBUG && RakuPhotoMail.DEBUG_PROTOCOL_IMAP) {
                    if (sensitive && !RakuPhotoMail.DEBUG_SENSITIVE) {
                        Log.v(RakuPhotoMail.LOG_TAG, getLogId() + ">>> "
                                + "[Command Hidden, Enable Sensitive Debug Logging To Show]");
                    } else {
                        Log.v(RakuPhotoMail.LOG_TAG, getLogId() + ">>> " + commandToSend);
                    }
                }
        ...
        public List<ImapResponse> executeSimpleCommand(String command) throws IOException, ImapException,
                MessagingException {
        ...
        public List<ImapResponse> executeSimpleCommand(String command, boolean sensitive,
                                                       UntaggedHandler untaggedHandler) throws IOException, ImapException, MessagingException {
            ...
            // if (RakuPhotoMail.DEBUG)
            // Log.v(RakuPhotoMail.LOG_TAG, "Sending IMAP command " +
            // commandToLog + " on connection " + getLogId());

            String tag = sendCommand(command, sensitive);
            // if (RakuPhotoMail.DEBUG)
            // Log.v(RakuPhotoMail.LOG_TAG, "Sent IMAP command " + commandToLog
            ...
            do {
                ...
                if (response.mTag != null && !response.mTag.equalsIgnoreCase(tag)) {
                    Log.w(RakuPhotoMail.LOG_TAG, "After sending tag " + tag
                            + ", got tag response from previous command " + response + " for " + getLogId());
    ...
    public class ImapFolderPusher extends ImapFolder implements UntaggedHandler {
        ...
        public ImapFolderPusher(ImapStore store, String name, PushReceiver nReceiver) {
            ...
            wakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "ImapFolderPusher "
                    + store.getAccount().getDescription() + ":" + getName());
        ...
        public void refresh() throws IOException, MessagingException {
            if (idling.get()) {
                wakeLock.acquire(RakuPhotoMail.PUSH_WAKE_LOCK_TIMEOUT);
                sendDone();
            }
        }
        ...
        public void start() {
            Runnable runner = new Runnable() {
                public void run() {
                    wakeLock.acquire(RakuPhotoMail.PUSH_WAKE_LOCK_TIMEOUT);
                    if (RakuPhotoMail.DEBUG)
                        Log.i(RakuPhotoMail.LOG_TAG, "Pusher starting for " + getLogId());
                    ...
                    while (!stop.get()) {
                        try {
                            ...
                            try {
                                ...
                                if (RakuPhotoMail.DEBUG)
                                    Log.i(RakuPhotoMail.LOG_TAG, "Got oldUidNext " + oldUidNext + " for "
                                            + getLogId());
                            } catch (Exception e) {
                                Log.e(RakuPhotoMail.LOG_TAG, "Unable to get oldUidNext for " + getLogId(), e);
                            }
                            ...
                            if (!conn.isIdleCapable()) {
                                stop.set(true);
                                receiver.pushError("IMAP server is not IDLE capable: " + conn.toString(),
                                        null);
                            ...
                            if (!stop.get() && mAccount.isPushPollOnConnect()
                                ...
                                List<ImapResponse> untaggedResponses = new ArrayList<ImapResponse>(
                                        storedUntaggedResponses);
                            ...
                            if (newUidNext == -1) {
                                if (RakuPhotoMail.DEBUG) {
                                    Log.d(RakuPhotoMail.LOG_TAG,
                                            "uidNext is -1, using search to find highest UID");
                                }
                                int highestUid = getHighestUid();
                                if (highestUid != -1) {
                                    if (RakuPhotoMail.DEBUG)
                                        Log.d(RakuPhotoMail.LOG_TAG, "highest UID = " + highestUid);
                            ...
                            if (newUidNext > startUid) {
                                ...
                                if (RakuPhotoMail.DEBUG)
                                    Log.i(RakuPhotoMail.LOG_TAG, "Needs sync from uid " + startUid + " to "
                                ...
                                while (storedUntaggedResponses.size() > 0) {
                                    if (RakuPhotoMail.DEBUG)
                                        Log.i(RakuPhotoMail.LOG_TAG,
                                                "Processing " + storedUntaggedResponses.size()
                                ...
                                if (RakuPhotoMail.DEBUG)
                                    Log.i(RakuPhotoMail.LOG_TAG, "About to IDLE for " + getLogId());
                            ...
                            try {
                                ...
                                Log.e(RakuPhotoMail.LOG_TAG, "Got exception while closing for exception for "
                                        + getLogId(), me);
                            }
                            if (stop.get()) {
                                Log.i(RakuPhotoMail.LOG_TAG,
                                        "Got exception while idling, but stop is set for " + getLogId());
                            } else {
                                receiver.pushError("Push error for " + getName(), e);
                                Log.e(RakuPhotoMail.LOG_TAG, "Got exception while idling for " + getLogId(),
                                        e);
                                ...
                                if (idleFailureCount.incrementAndGet() > IDLE_FAILURE_COUNT_LIMIT) {
                                    Log.e(RakuPhotoMail.LOG_TAG, "Disabling pusher for " + getLogId()
                                            + " after " + idleFailureCount.get() + " consecutive errors");
                    ...
                    try {
                        if (RakuPhotoMail.DEBUG)
                            Log.i(RakuPhotoMail.LOG_TAG, "Pusher for " + getLogId() + " is exiting");
                        close();
                    } catch (Exception me) {
                        Log.e(RakuPhotoMail.LOG_TAG, "Got exception while closing for " + getLogId(), me);
                    } finally {
                        wakeLock.release();
                    }
                }
        ...
        protected void handleUntaggedResponse(ImapResponse response) {
            if (response.mTag == null && response.size() > 1) {
                ...
                if (ImapResponseParser.equalsIgnoreCase(responseType, "FETCH")
                    ...
                    if (RakuPhotoMail.DEBUG)
                        Log.d(RakuPhotoMail.LOG_TAG, "Storing response " + response + " for later processing");
        ...
        protected void processUntaggedResponses(List<ImapResponse> responses) throws MessagingException {
            ...
            if (RakuPhotoMail.DEBUG)
                Log.d(RakuPhotoMail.LOG_TAG, "UIDs for messages needing flag sync are " + flagSyncMsgSeqs
                        + "  for " + getLogId());
        ...
        private void syncMessages(int end, boolean newArrivals) throws MessagingException {
            ...
            try {
                ...
                oldUidNext = pushState.uidNext;
                if (RakuPhotoMail.DEBUG)
                    Log.i(RakuPhotoMail.LOG_TAG, "Got oldUidNext " + oldUidNext + " for " + getLogId());
            } catch (Exception e) {
                Log.e(RakuPhotoMail.LOG_TAG, "Unable to get oldUidNext for " + getLogId(), e);
            }
            ...
            if (messageArray != null && messageArray.length > 0) {
                ...
                if (RakuPhotoMail.DEBUG)
                    Log.i(RakuPhotoMail.LOG_TAG, "Got newUid " + newUid + " for message " + end + " on "
                            + getLogId());
                int startUid = oldUidNext;
                ...
                if (newUid >= startUid) {
                    ...
                    if (RakuPhotoMail.DEBUG)
                        Log.i(RakuPhotoMail.LOG_TAG, "Needs sync from uid " + startUid + " to " + newUid
                                + " for " + getLogId());
                    List<Message> messages = new ArrayList<Message>();
        ...
        private void removeMessages(List<String> removeUids) {
            ...
            try {
                ...
                for (Message existingMessage : existingMessages) {
                    ...
                    Log.w(RakuPhotoMail.LOG_TAG, "Message with UID " + existingUid
                ...
                for (String uid : removeUids) {
                    ...
                    try {
                        ...
                        Log.e(RakuPhotoMail.LOG_TAG,
                                "Unable to set DELETED flag on message " + message.getUid());
                    }
                    messages.add(message);
                }
                receiver.messagesRemoved(this, messages);
            } catch (Exception e) {
                Log.e(RakuPhotoMail.LOG_TAG, "Cannot remove EXPUNGEd messages", e);
            }
        ...
        protected int processUntaggedResponse(int oldMessageCount, ImapResponse response,
            ...
            if (response.mTag == null && response.size() > 1) {
                try {
                    Object responseType = response.get(1);
                    if (ImapResponseParser.equalsIgnoreCase(responseType, "FETCH")) {
                        Log.i(RakuPhotoMail.LOG_TAG, "Got FETCH " + response);
                        int msgSeq = response.getNumber(0);

                        if (RakuPhotoMail.DEBUG)
                            Log.d(RakuPhotoMail.LOG_TAG, "Got untagged FETCH for msgseq " + msgSeq + " for "
                                    + getLogId());
                    ...
                    if (ImapResponseParser.equalsIgnoreCase(responseType, "EXPUNGE")) {
                        ...
                        if (RakuPhotoMail.DEBUG)
                            Log.d(RakuPhotoMail.LOG_TAG, "Got untagged EXPUNGE for msgseq " + msgSeq
                                    + " for " + getLogId());
                        ...
                        for (Integer msgSeqNumI : msgSeqs) {
                            if (RakuPhotoMail.DEBUG) {
                                Log.v(RakuPhotoMail.LOG_TAG, "Comparing EXPUNGEd msgSeq " + msgSeq + " to "
                                        + msgSeqNumI);
                            }
                            ...
                            if (msgSeqNum == msgSeq) {
                                ...
                                if (RakuPhotoMail.DEBUG) {
                                    Log.d(RakuPhotoMail.LOG_TAG, "Scheduling removal of UID " + uid
                                            + " because msgSeq " + msgSeqNum + " was expunged");
                                }
                                ...
                                String uid = msgSeqUidMap.get(msgSeqNum);
                                if (RakuPhotoMail.DEBUG) {
                                    Log.d(RakuPhotoMail.LOG_TAG, "Reducing msgSeq for UID " + uid + " from "
                    ...
                    Log.e(RakuPhotoMail.LOG_TAG, "Could not handle untagged FETCH for " + getLogId(), e);
                }
            }
        ...
        public void stop() {
            ...
            if (conn != null) {
                if (RakuPhotoMail.DEBUG)
                    Log.v(RakuPhotoMail.LOG_TAG, "Closing mConnection to stop pushing for " + getLogId());
                conn.close();
            } else {
        ...
        public void handleAsyncUntaggedResponse(ImapResponse response) {
            if (RakuPhotoMail.DEBUG)
                Log.v(RakuPhotoMail.LOG_TAG, "Got async response: " + response);

            if (stop.get()) {
                if (RakuPhotoMail.DEBUG)
                    Log.d(RakuPhotoMail.LOG_TAG, "Got async untagged response: " + response
                            + ", but stop is set for " + getLogId());

                try {
                    sendDone();
                } catch (Exception e) {
                    Log.e(RakuPhotoMail.LOG_TAG, "Exception while sending DONE for " + getLogId(), e);
                }
            } else {
                if (response.mTag == null) {
                    if (response.size() > 1) {
                        ...
                        if (ImapResponseParser.equalsIgnoreCase(responseType, "EXISTS")
                            ...
                            if (RakuPhotoMail.DEBUG)
                                Log.d(RakuPhotoMail.LOG_TAG, "Got useful async untagged response: "
                            ...
                            try {
                                ...
                                Log.e(RakuPhotoMail.LOG_TAG,
                                        "Exception while sending DONE for " + getLogId(), e);
                            }
                        }
                    } else if (response.mCommandContinuationRequested) {
                        if (RakuPhotoMail.DEBUG)
                            Log.d(RakuPhotoMail.LOG_TAG, "Idling " + getLogId());
    ...
    public class ImapPusher implements Pusher {
        ...
        public void stop() {
            if (RakuPhotoMail.DEBUG)
                Log.i(RakuPhotoMail.LOG_TAG, "Requested stop of IMAP pusher");

            synchronized (folderPushers) {
                for (ImapFolderPusher folderPusher : folderPushers.values()) {
                    try {
                        if (RakuPhotoMail.DEBUG)
                            Log.i(RakuPhotoMail.LOG_TAG, "Requesting stop of IMAP folderPusher "
                                    + folderPusher.getName());
                        folderPusher.stop();
                    } catch (Exception e) {
                        Log.e(RakuPhotoMail.LOG_TAG,
                                "Got exception while stopping " + folderPusher.getName(), e);
                    }
                }
                folderPushers.clear();
            }
                    ...
                    if (thisState.hasMoreTokens()) {
                        ...
                        if ("uidNext".equalsIgnoreCase(key) && thisState.hasMoreTokens()) {
                            ...
                            try {
                                ...
                                Log.e(RakuPhotoMail.LOG_TAG, "Unable to part uidNext value " + value, e);
                            }

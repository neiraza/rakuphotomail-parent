...
public class DecoderUtil {
    ...
    private static String decodeEncodedWord(String body, int begin, int end, Message message) {
        ...
        if (encodedText.length() == 0) {
            Log.w(RakuPhotoMail.LOG_TAG, "Missing encoded text in encoded word: '" + body.substring(begin, end) + "'");
            return null;
        }
        ...
        if (encoding.equalsIgnoreCase("Q")) {
            ...
            Log.w(RakuPhotoMail.LOG_TAG, "Warning: Unknown encoding in encoded word '" + body.substring(begin, end) + "'");
            return null;
        }
    }
}

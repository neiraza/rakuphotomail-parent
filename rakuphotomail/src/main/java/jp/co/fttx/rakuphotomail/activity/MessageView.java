...
import jp.co.fttx.rakuphotomail.view.AttachmentView;
import jp.co.fttx.rakuphotomail.view.ToggleScrollView;
import jp.co.fttx.rakuphotomail.view.SingleMessageView;
import jp.co.fttx.rakuphotomail.view.AttachmentView.AttachmentFileDownloadCallback;

import java.io.File;
import java.util.*;

public class MessageView extends RakuPhotoActivity implements OnClickListener {
    private static final String EXTRA_MESSAGE_REFERENCE = "jp.co.fttx.rakuphotomail.MessageView_messageReference";
    private static final String EXTRA_MESSAGE_REFERENCES = "jp.co.fttx.rakuphotomail.MessageView_messageReferences";
    private static final String EXTRA_NEXT = "jp.co.fttx.rakuphotomail.MessageView_next";
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        } else if (RakuPhotoMail.zoomControlsEnabled()) {
    ...
    public boolean onKeyDown(final int keyCode, final KeyEvent event) {
        ...
        case KeyEvent.KEYCODE_VOLUME_UP: {
            if (RakuPhotoMail.useVolumeKeysForNavigationEnabled()) {
                onNext();
                return true;
            }
        }
    ...
    public void onCreate(Bundle icicle) {
        ...
        if (RakuPhotoMail.DEBUG)
            Log.d(RakuPhotoMail.LOG_TAG, "MessageView got message " + mMessageReference);
...
//       setOnClickListener(R.id.reply_all_scrolling);
    ...
    private void displayMessage(MessageReference ref) {
        ...
        if (RakuPhotoMail.DEBUG)
            Log.d(RakuPhotoMail.LOG_TAG, "MessageView displaying message " + mMessageReference);
        mAccount = Preferences.getPreferences(this).getAccount(mMessageReference.accountUuid);
        clearMessageDisplay();
    ...
    private void setupDisplayMessageButtons() {
        ...
        if (mController.isMoveCapable(mAccount)) {
            ...
            mArchive.setEnabled(!mMessageReference.folderName.equals(mAccount.getArchiveFolderName()) &&
                                !RakuPhotoMail.FOLDER_NONE.equalsIgnoreCase(mAccount.getArchiveFolderName()));
            ...
            mSpam.setEnabled(!mMessageReference.folderName.equals(mAccount.getSpamFolderName()) &&
                             !RakuPhotoMail.FOLDER_NONE.equalsIgnoreCase(mAccount.getSpamFolderName()));
    ...
    private void onDelete() {
        if (RakuPhotoMail.confirmDelete()) {
    ...
    private void onRefile(String dstFolder) {
        ...
        if (RakuPhotoMail.FOLDER_NONE.equalsIgnoreCase(dstFolder)) {
            return;
        }

        if (mAccount.getSpamFolderName().equals(dstFolder) && RakuPhotoMail.confirmSpam()) {
    ...
    private void showNextMessageOrReturn() {
        if (RakuPhotoMail.messageViewReturnToList()) {
            finish();
        } else {
            showNextMessage();
        }
    ...
    private void onFlag() {
        if (mMessage != null) {
            ...
            try {
                ...
                Log.e(RakuPhotoMail.LOG_TAG, "Could not set flag on local message", me);
            }
        }
    }
    ...
    protected void onNext() {
        ...
        mLastDirection = NEXT;
        disableButtons();
        if (RakuPhotoMail.showAnimations()) {
...
// (Issue 3319)            mController.setFlag(mAccount, mMessageReference.folderName, new String[] { mMessage.getUid() }, Flag.SEEN, false);
            try {
                ...
                Log.e(RakuPhotoMail.LOG_TAG, "Unable to unset SEEN flag on message", e);
            }
        }
    }
        ...
        runOnUiThread(new Runnable() {
            public void run() {
                ...
                try {
                    ...
                    if (Config.LOGV) {
                        Log.v(RakuPhotoMail.LOG_TAG, "loadMessageForViewBodyAvailable", e);
                    }
                }
            }
        });

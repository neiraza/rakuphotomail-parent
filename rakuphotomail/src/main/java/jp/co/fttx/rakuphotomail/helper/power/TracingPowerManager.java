package jp.co.fttx.rakuphotomail.helper.power;
...
public class TracingPowerManager {
    ...
    public static synchronized TracingPowerManager getPowerManager(Context context) {
        if (tracingPowerManager == null) {
            if (RakuPhotoMail.DEBUG) {
                Log.v(RakuPhotoMail.LOG_TAG, "Creating TracingPowerManager");
            }
    ...
    public class TracingWakeLock {
        ...
        public TracingWakeLock(int flags, String ntag) {
            ...
            if (RakuPhotoMail.DEBUG) {
                Log.v(RakuPhotoMail.LOG_TAG, "TracingWakeLock for tag " + tag + " / id " + id + ": Create");
            }
        }
        ...
        public void acquire(long timeout) {
            ...
            if (RakuPhotoMail.DEBUG) {
                Log.v(RakuPhotoMail.LOG_TAG, "TracingWakeLock for tag " + tag + " / id " + id + " for " + timeout + " ms: acquired");
            }
            raiseNotification();
            if (startTime == null) {
        ...
        public void acquire() {
            ...
            raiseNotification();
            if (RakuPhotoMail.DEBUG) {
                Log.w(RakuPhotoMail.LOG_TAG, "TracingWakeLock for tag " + tag + " / id " + id + ": acquired with no timeout.  K-9 Mail should not do this");
            }
            if (startTime == null) {
                startTime = System.currentTimeMillis();
            }
        ...
        public void release() {
            if (startTime != null) {
                ...
                if (RakuPhotoMail.DEBUG) {
                    Log.v(RakuPhotoMail.LOG_TAG, "TracingWakeLock for tag " + tag + " / id " + id + ": releasing after " + (endTime - startTime) + " ms, timeout = " + timeout + " ms");
                }
            } else {
                if (RakuPhotoMail.DEBUG) {
                    Log.v(RakuPhotoMail.LOG_TAG, "TracingWakeLock for tag " + tag + " / id " + id + ", timeout = " + timeout + " ms: releasing");
                }
            }
            cancelNotification();
            synchronized (wakeLock) {
                ...
                synchronized (timer) {
                    ...
                    timerTask = new TimerTask() {
                        ...
                        public void run() {
                            if (startTime != null) {
                                Long endTime = System.currentTimeMillis();
                                Log.i(RakuPhotoMail.LOG_TAG, "TracingWakeLock for tag " + tag + " / id " + id + ": has been active for "
                                        + (endTime - startTime) + " ms, timeout = " + timeout + " ms");

                            } else {
                                Log.i(RakuPhotoMail.LOG_TAG, "TracingWakeLock for tag " + tag + " / id " + id + ": still active, timeout = " + timeout + " ms");
                            }
                        }

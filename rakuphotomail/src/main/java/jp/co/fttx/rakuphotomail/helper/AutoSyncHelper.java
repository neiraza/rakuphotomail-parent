...
public class AutoSyncHelper {
    ...
    private static IAutoSync loadAutoSync() {
        ...
        if (sdkVersion == Build.VERSION_CODES.CUPCAKE) {
            className = "jp.co.fttx.rakuphotomail.helper.AutoSyncSdk3";
        } else if (sdkVersion == Build.VERSION_CODES.DONUT) {
            className = "jp.co.fttx.rakuphotomail.helper.AutoSyncSdk4";
        } else if (sdkVersion >= Build.VERSION_CODES.ECLAIR) {
            className = "jp.co.fttx.rakuphotomail.helper.AutoSyncSdk5";
        }
        ...
        try {
            ...
            autoSync.initialize(RakuPhotoMail.app);
            ...
            if (RakuPhotoMail.DEBUG) {
                Log.d(RakuPhotoMail.LOG_TAG, "Couldn't load method to get auto-sync state", e);
            }
        }
        return null;
    }

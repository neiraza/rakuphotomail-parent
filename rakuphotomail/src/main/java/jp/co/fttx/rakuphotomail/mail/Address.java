...
public class Address {
        ...
        try {
            ...
            for (int i = 0, count = parsedList.size(); i < count; i++) {
                ...
                if (address instanceof Mailbox) {
                    ...
                    Log.e(RakuPhotoMail.LOG_TAG, "Unknown address type from Mime4J: "
                          + address.getClass().toString());
                }
            }
        } catch (MimeException pe) {
            Log.e(RakuPhotoMail.LOG_TAG, "MimeException in Address.parse()", pe);
        }
        return addresses.toArray(EMPTY_ADDRESS_ARRAY);
    }
    ...
    public CharSequence toFriendly(final Contacts contacts) {
        if (!RakuPhotoMail.showCorrespondentNames()) {
            return mAddress;
            ...
            if (name != null) {
                if (RakuPhotoMail.changeContactNameColor()) {
                    ...
                    coloredName.setSpan(new ForegroundColorSpan(RakuPhotoMail.getContactNameColor()),
                                        0,
                                        coloredName.length(),

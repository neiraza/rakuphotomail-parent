...
public class Storage implements SharedPreferences {
	...
	private CopyOnWriteArrayList<OnSharedPreferenceChangeListener> listeners = new CopyOnWriteArrayList<OnSharedPreferenceChangeListener>();

	private int DB_VERSION = 2;
	private String DB_NAME = "preferences_storage";

	private ThreadLocal<ConcurrentHashMap<String, String>> workingStorage = new ThreadLocal<ConcurrentHashMap<String, String>>();
	private ThreadLocal<SQLiteDatabase> workingDB = new ThreadLocal<SQLiteDatabase>();
	...
	private SQLiteDatabase openDB() {
		...
		if (mDb.getVersion() == 1) {
			...
			if (accountUuids != null && accountUuids.length() != 0) {
				...
				for (String uuid : uuids) {
					try {
						String storeUriStr = Utility.base64Decode(readValue(
								mDb, uuid + ".storeUri"));
						String transportUriStr = Utility
								.base64Decode(readValue(mDb, uuid
										+ ".transportUri"));
						...
						if (transportUriStr != null) {
							...
							String usernameEnc = URLEncoder.encode(
									userInfoParts[0], "UTF-8");
							String passwordEnc = "";
							String authType = "";
							if (userInfoParts.length > 1) {
								passwordEnc = ":"
										+ URLEncoder.encode(userInfoParts[1],
												"UTF-8");
							}
						...
						if (newUserInfo != null) {
							URI newUri = new URI(uri.getScheme(), newUserInfo,
									uri.getHost(), uri.getPort(),
									uri.getPath(), uri.getQuery(),
									uri.getFragment());
							String newTransportUriStr = Utility
									.base64Encode(newUri.toString());
							writeValue(mDb, uuid + ".transportUri",
									newTransportUriStr);
						}
						...
						if (storeUriStr.startsWith("imap")) {
							...
							if (userInfoParts.length == 2) {
								String usernameEnc = URLEncoder.encode(
										userInfoParts[0], "UTF-8");
								String passwordEnc = URLEncoder.encode(
								...
								newUserInfo = authType + ":" + usernameEnc
										+ ":" + passwordEnc;
							}
						...
						if (newUserInfo != null) {
							URI newUri = new URI(uri.getScheme(), newUserInfo,
									...
									uri.getPath(), uri.getQuery(),
									uri.getFragment());
							String newStoreUriStr = Utility.base64Encode(newUri
									.toString());
							writeValue(mDb, uuid + ".storeUri", newStoreUriStr);
						}
					} catch (Exception e) {
						Log.e(RakuPhotoMail.LOG_TAG, "ooops", e);
					}
				}
			}
		...
		if (mDb.getVersion() != DB_VERSION) {
			Log.i(RakuPhotoMail.LOG_TAG, "Creating Storage database");
		...
		if (tmpStorage != null) {
			if (RakuPhotoMail.DEBUG) {
				Log.d(RakuPhotoMail.LOG_TAG,
						"Returning already existing Storage");
			}
			return tmpStorage;
		} else {
			if (RakuPhotoMail.DEBUG) {
				Log.d(RakuPhotoMail.LOG_TAG, "Creating provisional storage");
			}
			...
			if (oldStorage != null) {
				...
				if (RakuPhotoMail.DEBUG) {
					Log.d(RakuPhotoMail.LOG_TAG,
							"Returning the Storage we created");
				}
				return tmpStorage;
			}
		}
	...
	private void loadValues() {
		...
		Log.i(RakuPhotoMail.LOG_TAG, "Loading preferences from DB into Storage");
		Cursor cursor = null;
		...
		try {
			...
			cursor = mDb.rawQuery(
					"SELECT primkey, value FROM preferences_storage", null);
			while (cursor.moveToNext()) {
				String key = cursor.getString(0);
				String value = cursor.getString(1);
				if (RakuPhotoMail.DEBUG) {
					Log.d(RakuPhotoMail.LOG_TAG, "Loading key '" + key
							+ "', value = '" + value + "'");
				}
			...
			Log.i(RakuPhotoMail.LOG_TAG, "Preferences load took "
	...
	protected void remove(String key) {
		workingDB.get().delete("preferences_storage", "primkey = ?",
				new String[] { key });
	...
	// @Override
	public jp.co.fttx.rakuphotomail.preferences.Editor edit() {
		return new jp.co.fttx.rakuphotomail.preferences.Editor(this);
	}
	...
	private String readValue(SQLiteDatabase mDb, String key) {
		...
		String value = null;
		try {
			cursor = mDb.query("preferences_storage", new String[] { "value" },
					"primkey = ?", new String[] { key }, null, null, null);
	...
	private void writeValue(SQLiteDatabase mDb, String key, String value) {
		...
		if (result == -1) {
			Log.e(RakuPhotoMail.LOG_TAG, "Error writing key '" + key
					+ "', value = '" + value + "'");
		}
	}
}

...
public class Storage implements SharedPreferences {
    ...
    private SQLiteDatabase openDB() {
        ...
        if (mDb.getVersion() == 1) {
            Log.i(RakuPhotoMail.LOG_TAG, "Updating preferences to urlencoded username/password");
    ...
    public static Storage getStorage(Context context) {
        ...
        if (tmpStorage != null) {
            if (RakuPhotoMail.DEBUG) {
                Log.d(RakuPhotoMail.LOG_TAG, "Returning already existing Storage");
            }
            return tmpStorage;
        } else {
            if (RakuPhotoMail.DEBUG) {
            ...
            if (oldStorage != null) {
                if (RakuPhotoMail.DEBUG) {
                    Log.d(RakuPhotoMail.LOG_TAG, "Another thread beat us to creating the Storage, returning that one");
                }
                return oldStorage;
            } else {
                if (RakuPhotoMail.DEBUG) {
                    Log.d(RakuPhotoMail.LOG_TAG, "Returning the Storage we created");
                }
                return tmpStorage;
            }
        }
    ...
    private void loadValues() {
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                ...
                if (RakuPhotoMail.DEBUG) {
                    Log.d(RakuPhotoMail.LOG_TAG, "Loading key '" + key + "', value = '" + value + "'");
                }
            ...
            Log.i(RakuPhotoMail.LOG_TAG, "Preferences load took " + (endTime - startTime) + "ms");
        }
    }
    ...
    private void writeValue(SQLiteDatabase mDb, String key, String value) {
        ...
        if (result == -1) {
            Log.e(RakuPhotoMail.LOG_TAG, "Error writing key '" + key + "', value = '" + value + "'");
        }
    }
}

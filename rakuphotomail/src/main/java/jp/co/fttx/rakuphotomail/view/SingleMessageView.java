...
public class SingleMessageView extends LinearLayout {
	...
	public void setHeaders(final Message message, Account account) {
		try {
			...
			Log.e(RakuPhotoMail.LOG_TAG, "setHeaders - error", me);
		}
	}
	...
	public void displayMessageBody(Account account, String folder, String uid, Message message,
			PgpData pgpData) throws MessagingException {
		...
		if (text != null) {
			...
			if (Utility.hasExternalImages(text) && !showPictures()) {
				if ((account.getShowPictures() == Account.ShowPictures.ALWAYS)
						|| ((account.getShowPictures() == Account.ShowPictures.ONLY_FROM_CONTACTS) && mContacts
								.isInContacts(message.getFrom()[0].getAddress()))) {
	...
	public void loadBodyFromText(CryptoProvider cryptoProvider, PgpData pgpData, Message message,
			String emailText, String contentType) {
		if (mScreenReaderEnabled) {
			mAccessibleMessageContentView.loadDataWithBaseURL("http://", emailText, contentType, "utf-8",
					null);
		} else {
	...
	Message message, Account account, MessagingController controller, MessagingListener listener)
		...
		if (part.getBody() instanceof Multipart) {
			...
			// Inline parts with a content-id are almost certainly components of
			// an HTML message
	...
	public void setAttachmentCallback(AttachmentView.AttachmentFileDownloadCallback attachmentCallback) {
